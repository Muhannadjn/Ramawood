...
const translations = {
  en: {
    ...
    'team-shaymaa-role': 'Sales Manager & International Relations',
    'team-erik-role': 'Quality Control & Logistics Manager',
    'team-anna-role': 'Financial Controller & Compliance Officer',
    ...
    'contact-owner': 'Owner',
    'contact-owner-desc': 'Leading RamaWood with expertise in international trade',
    'contact-agent': 'Sales Agent',
    'contact-org': 'Organization Number',
    'contact-phone': 'Phone',
    'contact-address': 'Our Location',
    'contact-location-click': 'Click to open in maps',
    'contact-whatsapp': 'Start Conversation',
  ...
  ar: {
    ...
    'team-shaymaa-role': 'مديرة المبيعات والعلاقات الدولية',
    'team-erik-role': 'مدير مراقبة الجودة واللوجستيات',
    'team-anna-role': 'مراقب مالي وضابط امتثال',
    ...
    'contact-owner-desc': 'يقود راما وود بخبرة في التجارة الدولية',
    'contact-agent': 'وكيل المبيعات',
    'contact-org': 'رقم المنظمة',
    'contact-phone': 'الهاتف',
    'contact-address': 'موقعنا',
    'contact-location-click': 'اضغط لفتح الخرائط',
    'contact-whatsapp': 'ابدأ المحادثة',
  ...
  sv: {
    ...
    'team-shaymaa-role': 'Säljchef & internationella relationer',
    'team-erik-role': 'Kvalitetskontroll & logistikchef',
    'team-anna-role': 'Finanscontroller & compliance officer',
    ...
    'contact-owner': 'Ägare',
    'contact-owner-desc': 'Leder RamaWood med expertis inom internationell handel',
    'contact-agent': 'Säljagent',
    'contact-org': 'Organisationsnummer',
    'contact-phone': 'Telefon',
    'contact-address': 'Vår plats',
    'contact-location-click': 'Klicka för att öppna i kartor',
    'contact-whatsapp': 'Starta konversation',
    'contact-whatsapp-title': 'WhatsApp',
...
function initializeImageLoading() {
  ...
  images.forEach(img => {
    // Don't animate hero background and logo images
    if (img.classList.contains('hero-bg-img') || 
        img.classList.contains('logo-img') || 
        img.classList.contains('wood-img') ||
        img.classList.contains('about-furniture-img') ||
        img.classList.contains('team-img-rect')) {
      img.style.opacity = '1';
      return;
    }
    ...
    // Set initial opacity for other images
    img.style.opacity = '0';
    img.style.transition = 'opacity 0.3s ease';
    
    // Ensure images show after a short delay if load event doesn't fire
    setTimeout(() => {
...
function initializeAnimations() {
  ...
  // Apply animations to elements (exclude images and image containers)
  const animateElements = document.querySelectorAll(
    '.section-title, .section-subtitle, .product-card, .impact-item, .contact-card, .team-member, .about-text, .hero-content h1, .hero-content p, .hero-cta'
  );

  animateElements.forEach((el, index) => {
    // Don't animate image containers that contain critical images
    if (el.querySelector('.hero-bg-img, .logo-img, .wood-img, .about-furniture-img, .team-img-rect')) {
      return;
    }
...
function switchLanguage(lang) {
  ...
  elements.forEach(element => {
    ...
    // Check main translations first, then form translations
    const translation = (translations[lang] && translations[lang][key]) || 
                       (formTranslations[lang] && formTranslations[lang][key]);
    if (translation) {
...
function updateActiveLangButton(selectedLang) {
  const allLangButtons = document.querySelectorAll('.lang-btn');
...
function openLocation() {
  const address = "Gåsagången 19, 422 61 Hisings Backa, Sweden";
  ...
  if (isMobile) {
    ...
    if (/iPhone|iPad|iPod/i.test(navigator.userAgent)) {
      ...
      window.open(`intent://maps.google.com/maps?q=${encodedAddress}#Intent;scheme=https;package=com.google.android.apps.maps;end`, '_blank');
      setTimeout(() => {
        window.open(`https://maps.google.com/maps?q=${encodedAddress}`, '_blank');
      }, 1000);
    } else {
